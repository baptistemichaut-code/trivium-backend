...
def enrich_album(title, artist):
    ...
    url = f"https://itunes.apple.com/search?term={query}&media=music&entity=album&limit=1&country=FR"
...
def build_links(item, apple_music_url=None):
    ...
    if t == "ALBUM":
        return [
            {"name": "Spotify", "category": "Écouter l'album", "urlString": f"https://open.spotify.com/search/{enc_tc}/albums", "iconName": "music.note"},
            {"name": "Apple Music", "category": "Streaming Lossless", "urlString": apple_music_url or f"https://music.apple.com/fr/search?term={enc_tc}", "iconName": "apple.logo"},
            {"name": "Deezer", "category": "Streaming Hi-Fi", "urlString": f"https://www.deezer.com/search/{enc_tc}/album", "iconName": "play.circle.fill"}
        ]
    elif t == "FILM":
        return [
            {"name": "Où regarder en streaming", "category": "Netflix, Prime, Disney+...", "urlString": f"https://www.justwatch.com/fr/recherche?q={enc_t}", "iconName": "play.tv.fill"},
            {"name": "Apple TV", "category": "Location & Achat 4K", "urlString": f"https://tv.apple.com/fr/search?term={enc_t}", "iconName": "apple.logo"},
            {"name": "Canal+ VOD", "category": "Location & myCANAL", "urlString": f"https://vod.canalplus.com/recherche/{enc_t}", "iconName": "film.fill"}
        ]
    else:
        return [
            {"name": "Fnac", "category": "Livre papier (Broché / Poche)", "urlString": f"https://www.fnac.com/SearchResult/ResultList.aspx?Search={enc_tc}", "iconName": "book.closed.fill"},
            {"name": "Kindle", "category": "Édition numérique E-book", "urlString": f"https://www.amazon.fr/s?k={enc_tc}&i=digital-text", "iconName": "ipad.and.arrow.forward"},
            {"name": "Audible", "category": "Livre audio narré", "urlString": f"https://www.audible.fr/search?keywords={enc_tc}", "iconName": "headphones"}
        ]
